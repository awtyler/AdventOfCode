...
enum Input {
    ...
    static func getInput(isPartTwo: Bool = false) -> [(Int, Int)] {
        var items: [(Int,Int)] = []
        
        for line in input.split(separator: "\n").map({ String($0) }) {
            
            var newline = line.lowercased()
...
//                newline = newline.replacingOccurrences(of: "zero", with: "0")
            }
            
            let firstNumIndex = newline.asArray().firstIndex(where: {
                return "0123456789".contains($0) })
            let lastNumIndex = newline.asArray().lastIndex(where: { "0123456789".contains($0) })
            
            let firstnum = Int(newline.asArray()[firstNumIndex!])!
            let lastnum = Int(newline.asArray()[lastNumIndex!])!
            
            print(firstnum, lastnum, newline)
            
            items.append((firstnum, lastnum))
        }
        return items
    }
